...
export default function PricingPage() {
  ...
  const getDiscountMessage = (): string => {
    ...
    return language === "en" ? `Incl. ${percentage}% discount!` : `Incl. ${percentage}% korting!`
  }
  ...
  const starterSpecialDiscount =
    clientType === "company" ? (language === "en" ? "Temporarily from €265!" : "Tijdelijk vanaf €265!") : ""
  ...
  return (
    <main className="min-h-screen flex flex-col">
      ...
      <section className="py-8 md:py-12 bg-muted/30 lg:px-4">
        <div className="container">
          <div className="max-w-md mx-auto">
            <label htmlFor="client-type" className="block text-sm font-medium mb-2">
            ...
            <Select value={clientType} onValueChange={(value) => setClientType(value as ClientType)}>
              <SelectTrigger id="client-type" className="w-full">
                <SelectValue placeholder={language === "en" ? "Select client type" : "Selecteer klanttype"} />
              </SelectTrigger>
              <SelectContent>
                ...
                <SelectItem value="nonprofit">
                  <div className="flex items-center">
                    ...
                    <span>{language === "en" ? "Non-profit" : "Non-profit"}</span>
                  </div>
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <div className="grid gap-8 lg:gap-16 lg:grid-cols-3">
            ...
            <Card className="border-2 hover:border-green-500 transition-all duration-300 flex flex-col">
              <CardHeader>
                ...
                <CardDescription className="text-lg font-medium">{t("pricing.starter.tagline")}</CardDescription>
                <div className="mt-4">
              ...
              <CardContent className="flex-grow">
                ...
                <div className="mt-6 text-muted-foreground">{t("pricing.starter.description")}</div>
              </CardContent>
              <CardFooter>
            ...
            <Card className="border-2 border-green-500 shadow-lg relative flex flex-col">
              ...
              <CardHeader>
                ...
                <CardDescription className="text-lg font-medium">{t("pricing.professional.tagline")}</CardDescription>
                <div className="mt-4">
                  <div className="text-xl font-bold">
                    {language === "en"
              ...
              <CardContent className="flex-grow">
                ...
                <div className="mt-6 text-muted-foreground">{t("pricing.professional.description")}</div>
              </CardContent>
              <CardFooter>
                <Button asChild className="w-full bg-green-500 hover:bg-green-600 text-white">
                  <Link href="/contact">{t("pricing.cta")}</Link>
            ...
            <Card className="border-2 hover:border-green-500 transition-all duration-300 flex flex-col">
              <CardHeader>
                ...
                <CardDescription className="text-lg font-medium">{t("pricing.custom.tagline")}</CardDescription>
                <div className="mt-4">
                  <div className="text-xl font-bold">{t("pricing.custom.price")}</div>
                  {discountMessage && <div className="text-green-500 font-medium">{discountMessage}</div>}
                </div>
              </CardHeader>
              <CardContent className="flex-grow">
                <ul className="space-y-2 md:space-y-4">
                  {t("pricing.custom.features", { returnObjects: true }).map((feature, index) => (
                    <li key={index} className="flex items-start">
                      <Check className="h-5 w-5 text-green-500 mr-2 shrink-0 mt-0.5" />
                      <span>{feature}</span>
                    </li>
                ...
                <div className="mt-6 text-muted-foreground">{t("pricing.custom.description")}</div>
              </CardContent>
              <CardFooter>
                <Button asChild className="w-full bg-green-500 hover:bg-green-600 text-white">
                  <Link href="/contact">{t("pricing.cta")}</Link>
      ...
      <section className="py-12 md:py-24 bg-muted/50 lg:px-4">
        <div className="container">
          <h2 className="text-3xl font-bold mb-8 md:mb-12 text-center">{t("pricing.additional.title")}</h2>
          <div className="grid md:grid-cols-2 gap-8 md:gap-16 max-w-4xl mx-auto">
            ...
            <Card className="border-2 hover:border-green-500 transition-all duration-300">
              <CardHeader>
                <CardTitle>{t("pricing.additional.validation.title")}</CardTitle>
              </CardHeader>
              <CardContent>
