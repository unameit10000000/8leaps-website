...
export default function PricingPage() {
  ...
  const [mobilePredefinedAccordionOpen, setMobilePredefinedAccordionOpen] =
    useState(false);
  const [mobileTierAccordionOpen, setMobileTierAccordionOpen] = useState(false);
  const [mobileDevTypeAccordionOpen, setMobileDevTypeAccordionOpen] =
    useState(false);
  ...
  const [selectedPredefinedPackage, setSelectedPredefinedPackage] =
    useState<PredefinedPackageType | null>(null);
  const [predefinedPackageClientType, setPredefinedPackageClientType] =
    useState<ClientType>("company");
  const [predefinedPackagePrice, setPredefinedPackagePrice] = useState<
    number | null
  >(null);
  const [predefinedPackageMonthly, setPredefinedPackageMonthly] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(DATA.predefinedPackages).length > 0
    ) {
      ...
      if (visiblePackages.length > 0) {
        setSelectedPredefinedPackage(
          visiblePackages[0][0] as PredefinedPackageType
        );
      }
    }
  ...
  useEffect(() => {
    if (selectedPredefinedPackage) {
      ...
      setPredefinedPackageMonthly(
        monthlyPrice > 0 ? Math.round(monthlyPrice) : null
      );
    } else {
      setPredefinedPackagePrice(null);
      setPredefinedPackageMonthly(null);
    ...
    selectedPredefinedPackage,
    predefinedPackageClientType,
    DATA.discounts.nonprofit,
    DATA.discounts.student,
  ]);
  ...
  useEffect(() => {
    ...
    // Check if consultation is needed
    if (
      (tierData && tierData.isConsultation) ||
      (devTypeData && devTypeData.isConsultation)
    ) {
      setTotalPrice(null);
      setMonthlyPrice(null);
      setNeedsConsultation(true);
  ...
  useEffect(() => {
    if (step === 3 && selectedTier && !selectedDevType) {
      ...
      const compatibleDevType = Object.entries(DATA.devTypes).find(
        ([key, devType]) => devType.compatibleTiers.includes(selectedTier)
      );
  ...
  const handlePredefinedPackageRequest = () => {
    ...
    console.log("Request submitted:", {
      ...
      price:
        predefinedPackagePrice !== null
          ? `€${predefinedPackagePrice}`
          : "Consultation needed",
      monthlyPrice:
        predefinedPackageMonthly !== null
          ? `€${predefinedPackageMonthly}/month`
          : null,
    });
  ...
  const getClientTypeTitle = (type: ClientType): string => {
    if (type === "company")
      return language === "en" ? "Company" : t("pricing.company");
    if (type === "student")
      return language === "en" ? "Student" : t("pricing.student");
  ...
  const getClientTypeDescription = (type: ClientType): string => {
    if (type === "company")
      return language === "en"
        ? "Standard pricing for businesses of all sizes."
        : t("pricing.company.desc");
    if (type === "student")
      return language === "en"
        ? "25% discount for students with valid ID."
        : t("pricing.student.desc");
    return language === "en"
      ? "50% discount for registered non-profit organizations."
      : t("pricing.nonprofit.desc");
  };

  // Client type cards for predefined packages
  const clientTypeCards = Object.entries(CLIENT_TYPE_INFO).map(
    ([key, info]) => (
      <div
        key={key}
        className={`p-4 rounded-lg border-2 cursor-pointer h-full ${
          predefinedPackageClientType === key
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-medium">
            {getClientTypeTitle(key as ClientType)}
          </h3>
          {info.discount && (
            <Badge className="bg-green-500 text-xs">-{info.discount}</Badge>
          )}
        </div>
        <p className="text-xs text-muted-foreground">
          {getClientTypeDescription(key as ClientType)}
        </p>
      </div>
    )
  );
  ...
  const calculatorClientTypeCards = Object.entries(CLIENT_TYPE_INFO).map(
    ([key, info]) => (
      <div
        key={key}
        className={`p-4 rounded-lg border-2 cursor-pointer h-full ${
  ...
  const getTierName = (tier: TierType): string => {
    if (tier === "starter")
      return language === "en" ? "Starter" : t("pricing.starter.title");
  ...
  const getDevTypeName = (devType: DevType): string => {
    if (devType === "iDontKnow")
      return language === "en"
        ? "I don't know"
        : t("pricing.tech.idontknow.details");
    if (devType === "frontendFull")
      return language === "en" ? "Frontend Full" : t("pricing.tech.frontend");
    if (devType === "wpCmsHeadless")
      return language === "en"
        ? "WP CMS (headless) + flexible frontend"
        : t("pricing.tech.wpheadless");
    if (devType === "wpCmsFull")
      return language === "en" ? "WP CMS Full" : t("pricing.tech.wpfull");
    return language === "en"
      ? "In consultation"
      : t("pricing.tech.consultation");
  };
  ...
  const getPackageName = (pkg: PackageType): string => {
    if (pkg === "integrations")
      return language === "en"
        ? "Integrations (WP Plugins)"
        : t("pricing.package.integrations");
    if (pkg === "contactForms")
      return language === "en"
        ? "Contact Forms"
        : t("pricing.package.contactforms");
    if (pkg === "setup")
      return language === "en" ? "Setup" : t("pricing.package.setup");
  ...
  const getPackageDescription = (pkg: PackageType): string => {
    if (pkg === "setup")
      return language === "en"
        ? "Domain, hosting & email (one-time fee)"
        : t("pricing.package.setup.desc");
    if (pkg === "maintenance")
      return language === "en"
  ...
  const predefinedPackageCards = Object.entries(DATA.predefinedPackages)
    ...
    .map(([key, pkg], index) => {
        ...
        return (
          <div
            ...
            className={`border-2 rounded-lg overflow-hidden transition-all h-full ${
              isMobile && selectedPredefinedPackage === key
                ? "border-green-500"
                : "border-border"
            }`}
            onClick={() =>
          ...
          >
            ...
            <CustomAccordion
              ...
              isOpen={
                isMobile
                  ? mobilePredefinedAccordionOpen
                  : openAccordions[`predefined-${index}`]
              }
              onToggle={(isOpen) =>
                handlePredefinedAccordionToggle(`predefined-${index}`, isOpen)
              }
      ...
      const needsConsultation =
        tierData.isConsultation || devTypeData.isConsultation;
      ...
      if (predefinedPackageClientType === "student") {
        ...
        displayMonthlyPrice = Math.round(
          displayMonthlyPrice * (1 - DATA.discounts.student)
        );
      } else if (predefinedPackageClientType === "nonprofit") {
        displayPrice = Math.round(
          displayPrice * (1 - DATA.discounts.nonprofit)
        );
        displayMonthlyPrice = Math.round(
          displayMonthlyPrice * (1 - DATA.discounts.nonprofit)
        );
      }
                ...
                <div className="text-sm text-muted-foreground mt-1">
                  ...
                  {predefinedPackageClientType !== "company" && (
                    <span className="text-green-500 text-xs ml-1">
                      ({getDiscountPercentage(predefinedPackageClientType)}%{" "}
                ...
                {tierData.inclusions.map((inclusion, idx) => (
                  <li key={idx} className="flex items-start">
                    ...
                    <span>
                      {language === "en"
                        ? inclusion
                        : t(`pricing.tier.${pkg.tier}.inclusion.${idx}`) ||
                          inclusion}
                    </span>
                  </li>
                ))}
  ...
  function calculatePackagePrice(
    pkg: typeof DATA.predefinedPackages.basic
  ): number {
    const tierData = DATA.tiers[pkg.tier];
    const devTypeData = DATA.devTypes[pkg.devType];
  ...
  function hasMonthlyFee(pkg: typeof DATA.predefinedPackages.basic): boolean {
    return pkg.packages.some(
      (packageKey) => DATA.packages[packageKey].isMonthly
    );
  }
  ...
  const tierCards = Object.entries(DATA.tiers).map(([key, tier], index) => (
    ...
    >
      ...
      <CustomAccordion
        title={t("pricing.view.details")}
        isOpen={
          isMobile ? mobileTierAccordionOpen : openAccordions[`tier-${index}`]
        }
        onToggle={(isOpen) =>
          handleTierAccordionToggle(`tier-${index}`, isOpen)
        }
        titleClassName="py-2 px-4 text-sm"
      >
        <ul className="space-y-2">
          {tier.inclusions.map((inclusion, idx) => (
            <li key={idx} className="flex items-start">
              ...
              <span>
                {language === "en"
                  ? inclusion
                  : t(`pricing.tier.${key}.inclusion.${idx}`) || inclusion}
              </span>
            </li>
          ))}
        </ul>
  ...
  const devTypeCards = Object.entries(DATA.devTypes).map(
    ([key, devType], index) => (
      <div
        key={key}
        className={`border-2 rounded-lg overflow-hidden transition-all h-full ${
      ...
      >
        <div
          className={`p-5 flex items-center justify-between ${
            isDevTypeCompatible(key as DevType) ? "cursor-pointer" : ""
          }`}
          onClick={() =>
            isDevTypeCompatible(key as DevType) &&
            setSelectedDevType(key as DevType)
          }
        >
          <div>
            <h3 className="font-bold">{getDevTypeName(key as DevType)}</h3>
            <div className="text-lg font-bold mt-1">
              {devType.isConsultation
                ? renderConsultationText()
                : `€${devType.price}`}
            </div>
          </div>
        ...
        <CustomAccordion
          ...
          isOpen={
            isMobile
              ? mobileDevTypeAccordionOpen
              : openAccordions[`dev-${index}`]
          }
          onToggle={(isOpen) =>
            handleDevTypeAccordionToggle(`dev-${index}`, isOpen)
          }
          titleClassName="py-2 px-4 text-sm"
        >
          <div>
            <p className="text-sm text-muted-foreground">
              {t("pricing.compatible.with")}
            </p>
            ...
            {/* {language === "nl" && (
              <p className="text-sm text-muted-foreground mt-2">
                {t(`${getDevTypeName(key as DevType)}`) || ""}
              </p>
            )} */}
          </div>
        </CustomAccordion>
        ...
        <>
          ...
          <div className="flex justify-between text-lg font-bold">
            ...
            <span>
              {predefinedPackagePrice !== null
                ? `€${predefinedPackagePrice}`
                : t("pricing.consultation.needed")}
              {predefinedPackageMonthly !== null &&
                ` + €${predefinedPackageMonthly}${
                  language === "en" ? "/month" : "/maand"
                }`}
            </span>
          </div>
          ...
          {selectedTier && (
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                ...
                {DATA.tiers[selectedTier].price !== null &&
                  ` (€${DATA.tiers[selectedTier].price})`}
              </span>
            </div>
          )}
          ...
          {selectedDevType && (
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                ...
                {DATA.devTypes[selectedDevType].price !== null &&
                  ` (€${DATA.devTypes[selectedDevType].price})`}
              </span>
            </div>
          )}
          ...
          <div className="flex justify-between text-lg font-bold">
            ...
            <span>
              {needsConsultation
                ...
                : ""}
              {monthlyPrice !== null &&
                ` + €${monthlyPrice}${language === "en" ? "/month" : "/maand"}`}
            </span>
          </div>
        </>
      )}
      ...
      <div className="container py-12">
        ...
        >
          <TabsList className="grid w-full grid-cols-2 mb-8">
            <TabsTrigger value="predefined-packages">
              {t("pricing.predefined.packages")}
            </TabsTrigger>
            <TabsTrigger value="calculate">
              {t("pricing.custom.calculator")}
            </TabsTrigger>
          </TabsList>
                ...
                <div className="mb-4">
                  <Carousel
                    ...
                    onSlideChange={(index) => {
                      ...
                      // Get visible packages based on client type
                      const visiblePackages = Object.entries(
                      ...
                      if (visiblePackages[index]) {
                        setSelectedPredefinedPackage(
                          visiblePackages[index][0] as PredefinedPackageType
                        );
                      }
                    }}
                  >
                      ...
                      onClick={() => {
                        ...
                        if (selectedPredefinedPackage === "mvp") {
                          window.location.href =
                            DATA.predefinedPackages.mvp.redirectTo;
                        } else {
              ...
              <div className="flex justify-between items-center max-w-3xl mx-auto">
                <div
                  className={`flex flex-col items-center ${
                    step >= 1 ? "text-green-500" : "text-muted-foreground"
                  } cursor-pointer`}
                  onClick={() => setStep(1)}
                >
                  <div
                ...
                <div
                  className={`flex-1 h-0.5 ${
                    step >= 2 ? "bg-green-500" : "bg-muted-foreground"
                  }`}
                ></div>
                <div
                  className={`flex flex-col items-center ${
                    step >= 2 ? "text-green-500" : "text-muted-foreground"
                  } cursor-pointer`}
                  onClick={() =>
                    clientType
                      ? setStep(2)
                      : setErrors(["Please select a client type first"])
                  }
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
                ...
                <div
                  className={`flex-1 h-0.5 ${
                    step >= 3 ? "bg-green-500" : "bg-muted-foreground"
                  }`}
                ></div>
                <div
                  className={`flex flex-col items-center ${
                    step >= 3 ? "text-green-500" : "text-muted-foreground"
                  } cursor-pointer`}
                  onClick={() =>
                    selectedTier
                      ? setStep(3)
                      : setErrors(["Please select a tier first"])
                  }
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
                  ...
                  <span className="text-sm mt-1">
                    {t("pricing.step.technology")}
                  </span>
                </div>
                <div
                  className={`flex-1 h-0.5 ${
                    step >= 4 ? "bg-green-500" : "bg-muted-foreground"
                  }`}
                ></div>
                <div
                  className={`flex flex-col items-center ${
                    step >= 4 ? "text-green-500" : "text-muted-foreground"
                  } cursor-pointer`}
                  onClick={() =>
                    selectedDevType
                      ? setStep(4)
                      : setErrors(["Please select a technology approach first"])
                  }
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
                      ...
                      {isMobile ? (
                        <Carousel
                          ...
                          onSlideChange={(index) => {
                            ...
                            if (
                              selectedTier &&
                              DATA.devTypes[devType].compatibleTiers.includes(
                                selectedTier
                              )
                            ) {
                        ...
                        {Object.entries(DATA.packages).map(([key, pkg]) => (
                          <div
                            ...
                            className={`p-4 border-2 rounded-lg ${
                              !isPackageCompatible(key as PackageType)
                                ? "opacity-50"
                                : ""
                            }`}
                          >
                            <div className="flex justify-between items-center">
                              <div>
                                <div className="flex items-center gap-2">
                                  <h3 className="font-bold">
                                    {getPackageName(key as PackageType)}
                                  </h3>
                              ...
                              <Checkbox
                                ...
                                onCheckedChange={() =>
                                  isPackageCompatible(key as PackageType) &&
                                  togglePackage(key as PackageType)
                                }
                                disabled={
                                  !isPackageCompatible(key as PackageType)
                                }
            ...
            >
              {isSubmitting ? (
                <span className="flex items-center">
                  ...
                  selectedPredefinedPackage &&
                  selectedPredefinedPackage !== "mvp" &&
                  (DATA.predefinedPackages[selectedPredefinedPackage] as any)
                    .tier &&
                  (DATA.tiers[
                    (DATA.predefinedPackages[selectedPredefinedPackage] as any)
                      .tier
